...
namespace Seikilos.LargeTextFileIndexerLib
{
    public class LargeFileIndexer
    {
        ...
        public Task IndexAsync(string fileToRead, string indexFileToWrite)
        {
            ...
            {
                return IndexAsync(inputStream, outputStream);
            }
        }
        ...
        /// </summary>
        /// <param name="inputStream">Arbitrary stream read byte by byte. The stream is not disposed after reading</param>
        ...
        public Task IndexAsync(Stream inputStream, Stream indexOutputStream)
        {
            ...
            var indexNext = 0L;
            var streamReader = new BinaryReader(inputStream);
